...
def testcase(func):
    def wrapper(tester, *args, **kwargs):
        ...
        start_sep = "=" * math.floor((80 - len(title) - 1) / 2)
        end_sep = "=" * (
            80 - (len(start_sep) + len(title)) - 1
        )
...
def udp_err_in_dname_too_long(tester):
    ...
    tester.send_message(b"\x04\x00\x01" + b"X" * 1024 + b"\x00" + b"X" * 1401 + b"\x00")
