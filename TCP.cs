        ...
        private void ProcessServerReply(string reply)
        {
            switch (reply)
            {
                ...
                case string r when r.ToUpper().StartsWith("ERR FROM"):
                    ...
                    Console.Error.WriteLine($"ERR FROM{errDisplayName}: {errMessage}".Trim());
                    Disconnect();
                    break;

                case string r when r.ToUpper().StartsWith("MSG FROM") && r.ToUpper().Contains("IS"):
                    ...
                    if (!Regex.IsMatch(messageDisplayName, displayRegex) || !Regex.IsMatch(messageContent, messageRegex))
                    {
                        ...
                        Console.WriteLine($"{messageDisplayName}: {messageContent}".Trim());
                    break;

                case string r when r.ToUpper().Contains("BYE"):                   
                    Disconnect();
                    break;
